...
export default function ReviewerSidebar() {
  ...
  const navItems = [
    ...
    { name: "Pending Reviews", href: "/reviewer/pending", icon: Search },
    { name: "Completed", href: "/reviewer/completed", icon: ClipboardCheck },
  ...
  return (
    <div className="w-64 bg-white border-r border-slate-100 flex flex-col h-full shadow-sm relative z-10">
      ...
      <div className="flex-1 overflow-y-auto py-6 px-4 space-y-2">
        {navItems.map((item) => {
          const isActive = pathname === item.href || (item.href !== "/reviewer/dashboard" && pathname?.startsWith(item.href));
          return (
