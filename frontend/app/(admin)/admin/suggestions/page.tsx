...
import { Search, Calendar, CheckCircle2, XCircle, Clock, AlertCircle, MessageSquare, ShieldCheck } from "lucide-react";
...
export default function AdminSuggestionsPage() {
  ...
  const handleDecision = async (id: string, status: string) => {
    if (!actionRemarks && (status === "CHANGES_REQUESTED" || status === "REJECTED")) {
      alert("Please provide remarks/reason for this decision.");
      return;
    }

    setActionLoading(true);
    try {
      const res = await api.patch(`/journals-suggestions/${id}/decision`, {
        status,
  ...
  return (
    <div className="p-4 md:p-8 space-y-4 md:space-y-8 max-w-7xl mx-auto">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1">Review journal proposals recommended by Editors and record the final administrative decision.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="p-12 text-center text-slate-400">No suggestions found in queue.</div>
            ) : (
        ...
        {/* Details and Decision Panel Column */}
          ...
          {selectedSuggestion ? (
            <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-6 sticky top-8">
              ...
              <div className="space-y-2">
                <h4 className="font-bold text-slate-700 text-sm">Workflow Trail</h4>
                <div className="border border-slate-100 rounded-2xl p-4 bg-slate-50/50 space-y-3 max-h-48 overflow-y-auto">
                  {selectedSuggestion.history?.map((h: any) => (
                    <div key={h.id} className="text-xs space-y-0.5 border-b border-slate-100 pb-2 last:border-b-0 last:pb-0">
                      <div className="flex justify-between font-bold text-slate-800">
                        <span>{h.status.replace(/_/g, " ")}</span>
                        <span className="font-normal text-slate-400">{new Date(h.createdAt).toLocaleDateString()}</span>
                      </div>
                      <p className="text-slate-500">By {h.user?.name} ({h.user?.role})</p>
                      <p className="text-slate-600 italic">"{h.remarks || "No comments"}"</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Admin Actions */}
              {!["APPROVED", "REJECTED", "CLOSED"].includes(selectedSuggestion.status) ? (
                <div className="border-t border-slate-100 pt-6 space-y-4">
                  <h3 className="font-bold text-slate-800 text-sm flex items-center gap-1.5">
                    <ShieldCheck size={16} className="text-teal-600" /> Administrative Action
                  </h3>

                  <textarea
                    rows={3}
                    placeholder="Provide decision comments, requirements or rejection remarks..."
                    value={actionRemarks}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => handleDecision(selectedSuggestion.id, "APPROVED")}
                      disabled={actionLoading}
                      className="p-2.5 bg-green-600 hover:bg-green-700 text-white text-xs font-bold rounded-xl transition-all cursor-pointer"
                    >
                      Approve Proposal
                    </button>
                    <button
                      onClick={() => handleDecision(selectedSuggestion.id, "REJECTED")}
                      disabled={actionLoading}
                      className="p-2.5 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-xl transition-all cursor-pointer"
                    >
                      Reject Proposal
                    </button>
                    <button
                      onClick={() => handleDecision(selectedSuggestion.id, "CHANGES_REQUESTED")}
                      disabled={actionLoading}
                    ...
                    <button
                      onClick={() => handleDecision(selectedSuggestion.id, "UNDER_EDITOR_REVIEW")}
                      disabled={actionLoading}
                      className="p-2.5 bg-slate-500 hover:bg-slate-600 text-white text-xs font-bold rounded-xl transition-all cursor-pointer"
                    >
                      Return to Editor
                    </button>
                  </div>
                ...
                <div className="border-t border-slate-100 pt-6 text-center text-xs text-slate-500 italic">
                  Decision recorded. Current status: {selectedSuggestion.status.replace(/_/g, " ")}.
                </div>
              )}
            </div>
          ) : (
            <div className="h-full border-2 border-dashed border-slate-100 rounded-3xl p-12 text-center text-slate-400 flex flex-col items-center justify-center min-h-[300px]">
              <p className="font-semibold text-sm">No suggestion selected</p>
              <p className="text-xs text-slate-400 mt-1">Select a proposal from the queue to run administrative actions.</p>
            </div>
          )}
        </div>
      </div>
